-- Allow AI video cuts to start from an uploaded MP4 when YouTube blocks automated access.
...
BEGIN
  ...
  IF _video_url IS NULL OR trim(_video_url) !~* '^https?://' THEN
    RAISE EXCEPTION 'Envie um arquivo MP4 válido para gerar os cortes.';
  END IF;
  ...
  INSERT INTO public.video_cut_jobs (
    ...
    trim(_video_url),
    COALESCE(NULLIF(trim(_source_title), ''), 'Vídeo enviado'),
    'upload',
    trim(_video_url),
    NULLIF(trim(_source_title), ''),
    v_requested,
    v_requested,
    true,
    'queued',
...
REVOKE EXECUTE ON FUNCTION public.create_video_cut_upload_job(uuid, text, integer, boolean, text) FROM PUBLIC, anon;
