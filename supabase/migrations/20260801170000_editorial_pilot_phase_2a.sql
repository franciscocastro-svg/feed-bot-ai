-- Piloto Editorial Fase 2A: aplicação confirmada e idempotente de fontes e pautas.
-- A descoberta e a validação externa continuam na Edge Function discover-rss;
-- esta função recebe apenas os candidatos validados e grava o plano em uma transação.

CREATE TABLE IF NOT EXISTS public.editorial_pilot_applications (
  ...
  CONSTRAINT editorial_pilot_proposal_id_check
    CHECK (proposal_id ~ '^preview-[a-f0-9]{16}$'),
  CONSTRAINT editorial_pilot_profile_fingerprint_check
    CHECK (profile_fingerprint ~ '^[a-f0-9]{64}$'),
  CONSTRAINT editorial_pilot_application_unique
    UNIQUE (user_id, instagram_account_id, proposal_id)
);

ALTER TABLE public.editorial_pilot_applications ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "owners read editorial pilot applications"
  ON public.editorial_pilot_applications;
CREATE POLICY "owners read editorial pilot applications"
ON public.editorial_pilot_applications
FOR SELECT
TO authenticated
USING (auth.uid() = user_id);

REVOKE ALL ON public.editorial_pilot_applications FROM PUBLIC, anon, authenticated;
GRANT SELECT ON public.editorial_pilot_applications TO authenticated;
GRANT ALL ON public.editorial_pilot_applications TO service_role;
