...
- **Integrado no Git, ainda não publicado:** o PR [#60](https://github.com/franciscocastro-svg/feed-bot-ai/pull/60) foi integrado à `main` no merge `acc8363` e adicionou o Corte Editorial aos Cortes IA. O PR rascunho [#61](https://github.com/franciscocastro-svg/feed-bot-ai/pull/61), branch `codex/editorial-admin-beta` criada sobre `fd79e5d`, organiza a página nas subabas `Criar corte` e `Meus cortes`; dentro da criação, os formatos são `Corte tradicional`, `Corte com legendas` e `Corte editorial`. O editorial aparece como `Beta admin` somente para administradores, e banco, RPCs e Edge Function repetem essa autorização para impedir bypass da interface. O fluxo cria uma prévia 4:5 editável, usa transcrição e frames com guardrails factuais, exige confirmação para renderizar o vídeo final e bloqueia autopublicação. A migration, a nova Edge Function, o frontend e o worker ainda não foram implantados.
...
- O `Validate application` do PR rascunho #61 passou em 1m47s para a implementação e o primeiro registro documental da Beta administrativa.
...
Concluir a revisão do PR rascunho #61 e solicitar autorização separada para implantar a Beta administrativa, na ordem: migration, Edge de texto, worker VPS e frontend. Depois, testar com uma conta administradora e um vídeo real com fala, sem publicar; somente após esse aceite remover a restrição temporária de administrador e preparar o rollout aos clientes. A recaptura da matéria, o replay do Piloto e a correção do novo bloqueio da VPS permanecem atividades separadas.
