...
- correção local pendente de implantação torna o Gemini o transcritor padrão único, divide vídeos em blocos de 120 segundos, recupera JSON parcialmente truncado, limita espera/retry e mostra progresso por bloco; se nenhuma fala utilizável for obtida no modo editorial, o job falha com orientação clara em vez de produzir texto editorial sem base.
...
O smoke seguinte expôs que a VPS tinha somente Gemini, enquanto o worker ainda segmentava a fonte em blocos de 600 segundos. Respostas truncadas e timeouts mantiveram o primeiro job em 25% e o segundo na fila. A correção local reduz os blocos, aplica schema de resposta, parsing recuperável e progresso incremental. Nada dessa correção foi implantado e os dois jobs de teste ainda exigem cancelamento operacional antes do próximo smoke.
