...
- O PR [#51](https://github.com/franciscocastro-svg/feed-bot-ai/pull/51) foi integrado na `main` pelo merge `ad39d3e` e conecta o Perfil do Criador à descoberta real de RSS/monitoramento temático, com seleção, resumo e aplicação transacional/idempotente. A migration da entrega já foi aplicada; Edge Function e frontend ainda não foram publicados.
...
- A publicação de `discover-rss` pelo Supabase CLI foi interrompida antes de qualquer alteração porque não há sessão administrativa (`SUPABASE_ACCESS_TOKEN`) nesta máquina. O frontend não foi publicado e a flag de produção não foi ativada.
...
A Fase 2A do Piloto Editorial está integrada e validada na `main`, e sua migration já está aplicada. O próximo passo exato é autorizar uma sessão administrativa do Supabase para publicar somente `discover-rss`; depois, publicar o frontend e executar um smoke autenticado antes de decidir a ativação da flag em produção.
