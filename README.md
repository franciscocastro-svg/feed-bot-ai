...
- **Ajuste local pós-smoke, ainda não publicado:** a branch `codex/refine-editorial-cut-identity` exige pelo menos 20 segundos somente no Corte Editorial, desde a seleção da IA até a revisão e a proteção do banco. O cabeçalho passa a consultar nome, `@` e foto da conta efetivamente selecionada na Meta, rejeita respostas pertencentes a outro `@` e nunca reutiliza o logo de outra conta. O selo azul só aparece quando a API devolve confirmação explícita de verificação; saúde do token não é tratada como selo público. A migration `20260802200000_enforce_editorial_cut_identity_duration.sql` está apenas no repositório e não foi aplicada.
...
Concluir o CI e a revisão da branch `codex/refine-editorial-cut-identity`. Depois de aprovação, enviar a branch e abrir PR; migration, frontend e worker continuam sem implantação. O smoke posterior deve confirmar cortes de pelo menos 20 segundos, nome/foto/@ da conta selecionada e selo somente quando realmente confirmado, primeiro em Reel 9:16 e depois em Feed 4:5, sem render final automático, agendamento ou publicação. O bloqueio antigo da fila (`fbe6a2a`), a recaptura da matéria, o replay do Piloto e a correção definitiva do `SIGINT` permanecem atividades separadas.
