...
-- Mantem o primeiro item ativo e cancela copias que ja estejam na fila.
...
WITH active AS (
  ...
  SELECT
    id,
    row_number() OVER (
      PARTITION BY user_id, instagram_account_id, url_key
  ...
  SELECT
    id,
    row_number() OVER (
      PARTITION BY user_id, instagram_account_id, title_key
      ORDER BY scheduled_for ASC, created_at ASC, id ASC
    ) AS rn
  FROM active
  WHERE title_key IS NOT NULL
    AND length(title_key) >= 18
), duplicate_ids AS (
