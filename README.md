...
- **Corte Editorial pronto para smoke autenticado:** os PRs [#60](https://github.com/franciscocastro-svg/feed-bot-ai/pull/60), [#61](https://github.com/franciscocastro-svg/feed-bot-ai/pull/61), [#62](https://github.com/franciscocastro-svg/feed-bot-ai/pull/62) e [#64](https://github.com/franciscocastro-svg/feed-bot-ai/pull/64) foram integrados. A correção Bold/Clean e Reel 9:16 entrou no merge `5105bca`; a Lovable registrou a migration como `20260802164442_2b52a212-51a9-42c0-ad0f-681037be48ea.sql`, recarregou o PostgREST, publicou o frontend e reconciliou a `main` no merge automático `efc8d15`.
...
- **Correção local para o smoke mais recente:** o erro `invalid input syntax for type integer: "45.24"` foi reproduzido como incompatibilidade entre os limites decimais calculados pelo worker e as colunas inteiras de `video_cut_clips`. A branch `codex/fix-gemini-cut-transcription` agora persiste início/fim/duração inteiros, sem ultrapassar a duração física do arquivo. Para recuperar a velocidade de vídeos de 60 minutos, arquivos com 10 minutos ou 100 MB passam por uma única análise multimodal via Gemini Files API; somente os candidatos escolhidos são extraídos e transcritos para legenda/texto. Vídeos curtos preservam a transcrição segmentada de 120 segundos e a rota segmentada continua como contingência. Se a transcrição ainda falhar, o job preserva a prévia neutra com 0% de confiança e `Revisão necessária`, em vez de publicar ou falhar por completo. Esta etapa está apenas local: nenhum deploy, dado ou job foi alterado.
- **Novo resultado real observado às 15:47:** o job iniciado às 15:12 chegou a `Pronto para revisão` e gerou uma prévia editorial de 35 segundos. A interface sinalizou `Não foi possível transcrever o vídeo completo`, confiança 0% e texto neutro editável; isso comprova renderização/fallback, mas não aprovação da transcrição ou do conteúdo editorial. A lista identifica o pedido como `9:16`, enquanto a prévia informa `Feed 1080 × 1350`; o worker implantado ainda forçou 4:5. A branch local já remove essa imposição e precisa ser validada no próximo smoke 9:16.
...
Concluir o CI e revisar o segundo commit local da branch `codex/fix-gemini-cut-transcription`. A correção combina quatro garantias: timestamps inteiros compatíveis com o banco, Gemini Files API para analisar vídeos longos uma única vez, transcrição apenas dos trechos candidatos e fallback segmentado de 120 segundos. O worker continua respeitando a escolha editorial 4:5 ou 9:16.

Depois da aprovação: enviar a branch e abrir PR; só após merge e nova autorização, implantar o novo SHA e reiniciar exclusivamente `feedbot-cuts`. O job atual já chegou à revisão e não deve ser cancelado automaticamente. O smoke seguinte deve confirmar que o mesmo vídeo recebe transcrição/título reais; a prévia neutra continua como fallback seguro, sem render final, agendamento ou publicação. O bloqueio antigo da fila (`fbe6a2a`), a recaptura da matéria, o replay do Piloto e a correção definitiva do `SIGINT` permanecem atividades separadas.
