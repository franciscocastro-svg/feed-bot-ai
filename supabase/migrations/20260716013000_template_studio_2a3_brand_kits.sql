...
-- Account-scoped brand kits and safe recommendation inputs. Published
-- template versions remain immutable; applying a kit only writes a draft
-- snapshot through the existing Template Studio 2A.1 RPCs.
...
-- Seed one isolated kit per Instagram account. Identity remains in
-- account_settings for backwards compatibility with every renderer.
