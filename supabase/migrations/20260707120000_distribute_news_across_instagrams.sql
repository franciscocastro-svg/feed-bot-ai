-- Impede que o mesmo cliente tenha a mesma noticia ativa em mais de uma conta.
-- Historico publicado permanece intacto; a limpeza atinge somente fila/itens ativos.
...
WITH active_items AS (
  ...
  SELECT
    id,
    row_number() OVER (
      PARTITION BY user_id, dedupe_url_key
  ...
  SELECT
    id,
    row_number() OVER (
      PARTITION BY user_id, dedupe_title_key
      ORDER BY created_at ASC, id ASC
    ) AS rn
  FROM active_items
