...
-- Access is readable only by the matching authenticated user or service_role.
-- Allow-list paid states explicitly; every other Stripe state is denied.
...
begin
  ...
  -- A renewal failure commonly occurs exactly when current_period_end expires.
  -- Let the explicit past_due window decide access instead of defeating the
  -- 72-hour grace with a stale period timestamp.
...
-- Apply one Stripe subscription event and fenced-complete its webhook ledger in
-- the same PostgreSQL transaction. Reconciliation calls may pass NULL request_id
-- because they do not own a webhook ledger claim.
