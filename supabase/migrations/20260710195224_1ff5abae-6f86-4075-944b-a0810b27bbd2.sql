  ...
  SELECT public.has_role(_uid, 'admin'::public.app_role)
    OR EXISTS (
      ...
      WHERE s.user_id = _uid
        ...
        AND (COALESCE(s.current_period_end, s.expires_at) IS NULL
...
AS $$ SELECT public.has_active_entitlement(_uid); $$;
...
DO $$
DECLARE account_row record; new_secret_id uuid;
BEGIN
  FOR account_row IN
    SELECT id, access_token FROM public.instagram_accounts
    WHERE access_token IS NOT NULL AND btrim(access_token) <> '' AND token_secret_id IS NULL
  LOOP
    SELECT vault.create_secret(account_row.access_token,
      'instagram-account-' || account_row.id::text,
      'Instagram access token managed by Flux & Feed') INTO new_secret_id;
    UPDATE public.instagram_accounts
    SET token_secret_id = new_secret_id, access_token = NULL
...
BEGIN
  ...
  IF NEW.access_token IS NOT NULL AND btrim(NEW.access_token) <> '' THEN
    SELECT vault.create_secret(NEW.access_token,
...
RETURNS text LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public, vault
AS $$
...
GRANT SELECT (id, user_id, username, ig_user_id, page_id, niche, active, created_at,
  updated_at, custom_hashtags, token_expires_at, last_verified_at, verification_status)
...
BEGIN
  ...
  FROM public.user_subscriptions s WHERE s.user_id = owner_id LIMIT 1;
...
CREATE POLICY "video cut inputs owner insert" ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'video-cut-inputs'
...
CREATE OR REPLACE FUNCTION public.create_video_cut_upload_job_v2(
  _instagram_account_id uuid, _storage_path text, _requested_clips integer,
  _rights_confirmed boolean, _source_title text DEFAULT NULL,
  _format text DEFAULT 'reels', _subtitle_style text DEFAULT 'classic',
  _auto_publish boolean DEFAULT false, _remove_silences boolean DEFAULT true,
  _zoom_effect boolean DEFAULT false, _smart_crop boolean DEFAULT true,
  _formats text[] DEFAULT NULL, _hook_enabled boolean DEFAULT true
...
BEGIN
  ...
  IF _storage_path IS NULL OR _storage_path NOT LIKE expected_prefix || '%'
     OR _storage_path LIKE '%..%' OR lower(_storage_path) NOT LIKE '%.mp4' THEN
    RAISE EXCEPTION 'invalid private video storage path';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM storage.objects
    WHERE bucket_id = 'video-cut-inputs' AND name = _storage_path) THEN
    RAISE EXCEPTION 'uploaded video not found';
  END IF;

  created_job := public.create_video_cut_upload_job(
    _instagram_account_id, 'storage://video-cut-inputs/' || _storage_path,
    _requested_clips, _rights_confirmed, _source_title, _format, _subtitle_style,
    _auto_publish, _remove_silences, _zoom_effect, _smart_crop, _formats, _hook_enabled);

  UPDATE public.video_cut_jobs
  SET source_video_url = NULL, source_storage_bucket = 'video-cut-inputs',
...
BEGIN
  ...
  FROM public.worker_health h ORDER BY h.queue_mode;
END; $$;
...
CREATE OR REPLACE FUNCTION public.delete_instagram_account_data(
  _meta_user_id text, _confirmation_code text
...
BEGIN
  ...
  FROM public.instagram_accounts WHERE ig_user_id = _meta_user_id;

  SELECT COALESCE(array_agg(id), '{}'::uuid[]) INTO v_news_ids
  FROM public.news_items WHERE instagram_account_id = ANY(v_account_ids);
  ...
  RETURN jsonb_build_object('confirmation_code', _confirmation_code,
    'account_ids', v_account_ids, 'user_ids', v_user_ids, 'news_item_ids', v_news_ids);
END; $$;
