...
- **Corte Editorial em implantação controlada:** os PRs [#60](https://github.com/franciscocastro-svg/feed-bot-ai/pull/60) e [#61](https://github.com/franciscocastro-svg/feed-bot-ai/pull/61) foram integrados nos merges `acc8363` e `e433493`. A Lovable aplicou a migration como `20260802144135_c24d0a10-3467-4111-9436-659f3143145c.sql`, recarregou o PostgREST e implantou somente `regenerate-cut-editorial-text`; o merge automático `ad273b4` registrou a migration e os tipos gerados. O Preview já exibe `Criar corte`/`Meus cortes` e a opção `Beta admin`, mas o worker VPS ainda não recebeu o compositor e o frontend de produção não foi confirmado. Não executar novo smoke até o worker ser atualizado.
...
Validar no GitHub o novo escopo operacional `cuts-only`, auditar o bloqueio conhecido da VPS e implantar somente `feedbot-cuts` no SHA final aprovado, sem reiniciar o webhook. Depois, testar como administrador com um vídeo real com fala, sem agendar ou publicar; somente após esse aceite confirmar o frontend de produção e preparar o rollout aos clientes. A recaptura da matéria, o replay do Piloto e a correção definitiva do `SIGINT` da fila permanecem atividades separadas.
