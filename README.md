...
- **Programa de afiliados implantado:** o [PR #70](https://github.com/franciscocastro-svg/feed-bot-ai/pull/70), merge `c26dd9e01ef4540af3e2dbc28e798539d8b2a494`, entrega ativação exclusiva pelo admin, link `?ref=`, atribuição imutável de novos cadastros, painel privado com métricas agregadas e gestão administrativa de ativação/pausa. A migration `20260802230000_affiliate_referrals.sql` foi aplicada (registrada como `20260803032658`) e somente o frontend da `main` foi publicado. Nenhum plano, assinatura, pagamento, Edge Function, worker ou dado de cliente foi alterado, e nenhum afiliado de teste foi criado.
...
O programa de afiliados está implantado: merge `c26dd9e` sincronizado, migration `20260802230000_affiliate_referrals.sql` aplicada como `20260803032658` e frontend da `main` publicado. As verificações confirmaram RLS ativo, zero policies, acesso direto revogado, as quatro RPCs com guardas de administrador e nenhuma alteração em assinatura, plano ou pagamento. O próximo passo é ativar o primeiro afiliado real pelo painel administrativo quando houver decisão comercial e acompanhar a primeira atribuição em uso normal. Comissão, saldo e pagamento continuam fora do escopo. Rollout do cancelamento, smoke editorial 4:5, bloqueio antigo da fila, recaptura da matéria, replay do Piloto e correção do `SIGINT` permanecem atividades separadas.
