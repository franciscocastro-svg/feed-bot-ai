...
- Estado: implementação, validação automatizada e três renders físicos concluídos; demonstração visual aprovada; subabas e acesso temporário `Beta admin` implementados com defesa na UI e no backend. Commits anteriores enviados para `origin/codex/editorial-ai-cut`; PR rascunho [#60](https://github.com/franciscocastro-svg/feed-bot-ai/pull/60) aberto contra `main`. Sem merge, migration, publicação ou deploy desta funcionalidade.
...
2. concluir os checks do novo head do PR #60 e mantê-lo como rascunho até autorização;
