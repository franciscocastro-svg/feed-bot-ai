...
- **Cancelamento de fila implementado localmente, ainda não implantado:** a branch `codex/cancel-video-cut-jobs` adiciona `Cancelar fila` somente para jobs `queued`, `analyzing` ou `processing`. A RPC proposta bloqueia a linha, valida proprietário/admin, marca apenas o job escolhido como `cancelled` e libera sua reserva diária de forma idempotente. O worker verifica cancelamento entre etapas pesadas, impede conclusão/autopublicação posterior e remove artefatos parciais; outros jobs e processos PM2 não são interrompidos. A migration `20260802220000_cancel_video_cut_jobs.sql` não foi aplicada e nenhum deploy foi feito.
- A branch foi enviada e o PR rascunho [#68](https://github.com/franciscocastro-svg/feed-bot-ai/pull/68) foi aberto contra `main`, sem conflitos. O check remoto `Validate application` aprovou o head funcional `67f2e59` em 2m02s. O PR não foi marcado como pronto, integrado ou implantado.
...
Revisar o PR rascunho #68, aguardar o check do registro documental final e então marcá-lo como pronto para revisão antes do merge. Somente após a integração, aplicar `20260802220000_cancel_video_cut_jobs.sql`, publicar o frontend e atualizar exclusivamente `feedbot-cuts` na VPS. Fazer um smoke com um job na fila e outro em processamento, confirmar que apenas o escolhido vira `cancelled`, que os créditos reservados voltam e que `Excluir` fica disponível. O bloqueio antigo da fila (`fbe6a2a`), a recaptura da matéria, o replay do Piloto e a correção definitiva do `SIGINT` permanecem atividades separadas.
