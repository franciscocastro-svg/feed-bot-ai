...
--
-- Deployment order is documented in ops/releases/phase-1e-a-2.json. This
-- migration does not change the legacy try_claim_payment_webhook_effect RPC;
-- the currently deployed webhook therefore remains in containment mode.
-- =============================================================================

-- Reconciler-only, atomic claim. It can recover only effect types whose retry
-- policy is idempotent. Verification e-mail and unknown effects never enter
-- this claim path, including after an expired processing lease.
...
-- Fail closed before replacing cron jobs. Values are checked but never copied
-- into the migration or cron.job command text.
