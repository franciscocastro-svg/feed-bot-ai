...
-- 1) Adicionar coluna format nos jobs e clips
ALTER TABLE public.video_cut_jobs
  ADD COLUMN IF NOT EXISTS format text NOT NULL DEFAULT 'reels'
  CHECK (format IN ('feed_square','feed_portrait','reels'));

ALTER TABLE public.video_cut_clips
  ADD COLUMN IF NOT EXISTS format text NOT NULL DEFAULT 'reels'
  CHECK (format IN ('feed_square','feed_portrait','reels'));

-- 2) Atualizar RPC de upload para aceitar formato
CREATE OR REPLACE FUNCTION public.create_video_cut_upload_job(
  ...
  _source_title text DEFAULT NULL::text,
  _format text DEFAULT 'reels'
)
RETURNS video_cut_jobs
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_format NOT IN ('feed_square','feed_portrait','reels') THEN
    RAISE EXCEPTION 'Formato inválido. Use feed_square, feed_portrait ou reels.';
  END IF;
...
-- 3) Atualizar RPC de YouTube para aceitar formato
